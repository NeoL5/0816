...
def activate_camera(video_address, num_of_blur, num_of_brighten, filter_enhance):
    video_placeholder = st.empty()
    # 打开摄像头，参数 0 代表第一个摄像头
    cap = cv2.VideoCapture(f"http://{video_address}/mjpegfeed")
...
def main_loop():
    ...
    ip_address = st.text_input('Please input the Wifi IP', max_chars=50, help='最大长度为50字符')
    port_address = st.text_input('Please input the port number', max_chars=50, help='最大长度为50字符')
    address = f'{ip_address}:{port_address}'
    ...
    while st.button('Confirm', key=unique_key):
